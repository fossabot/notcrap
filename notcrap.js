...
Object.defineProperties(HTMLElement.prototype,
    ...
    place:
 {
   get() {
     return function () {
       for (let a = this.parentNode.children, b = a.length, c = 0; c < b; c++) { if (a[c] == this) return c; }
       return -1;
     };
   },
 },
...
const ajax = function (a, b) {
  ...
  if (!a.url) throw 'You need to give an URL';
  const d = a.url,
	      e = (a.method || 'GET').toUpperCase();
  let f,
	    g = a.data;
  e == 'POST' &&
  (h = typeof g === 'string' ? g : Object.keys(g).map(j =>
    `${encodeURIComponent(j)}=${encodeURIComponent(g[j])}`).join('&')),
  (f = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP')).open(e, d),
  f.onreadystatechange = function () {
    f.readyState > 3 && f.status == 200 && c(f.responseText);
  },
  e == 'POST' && f.setRequestHeader('X-Requested-With', 'XMLHttpRequest'),
  f.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'),
  e == 'POST' ? f.send(h) : f.send();
};
...
Script = function (a) {
  ...
  this.update = function () {
    ...
    this.src ? this.scr.src = this.src : this.scr.innerHTML = b,
...
function refresh() {
  for (let a = get('*'), b = 0, c = a.length; b < c; b++) {
    a[b].style.getProperty = function (d) {
      return this[d];
    };
  }
}
