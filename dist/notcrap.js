...
function get(sel, arr) {
	...
	const d = [],
	l = !!sel && ("object" == typeof sel ? nc.toArr(sel) : Sizzle(sel.trim()));
	return d.push(...l),
	arr ? nc.toNodeList(d) : 1 === d.length ? d[0] : nc.toNodeList(d);
}

window.nc = window.notcrap = {
	version: "2.0.0",
	toArr: a => Array.isArray(a) ? [...a] : [a],
	// find a CSS path that uniquely selects this element
	buildIndexCSSPath: function (elem) {
		...
		return nc.buildIndexCSSPath(parent) + " > :nth-child(" + (Array.prototype.indexOf.call(parent.children, elem) + 1) + ")";
	},

	toNodeList: function (list) {
		if((!list)||list instanceof NodeList || list instanceof HTMLCollection)return list;
		// map all elements to CSS paths
		var names = list.map(function (elem) {
				return nc.buildIndexCSSPath(elem);
			});

		// join all paths by commas
		var superSelector = names.join(",");
		if(!superSelector) return list;
	...
	hide(el) {
		nc.setStyle(el, "display:none;visibility:hidden;");
	},
	show(el) {
		nc.setStyle(el, "display:block;visibility:visible;");
	},
	addClass(els, cls) {
		get(els, !0).forEach(el => {
	...
	funcs: {
		...
		addClass: {
			get: () => (function (cls) {
				nc.addClass(this, cls);
			})
		},
		removeClass: {
			get: () => (function (cls) {
				nc.removeClass(this, cls);
			})
		},
		toggleClass: {
			get: () => (function (cls) {
				nc.toggleClass(this, cls);
			})
		},
		hasClass: {
			get: () => (function (cls) {
				let el = this.length ? this[0] : this;
				return el.classList ? el.classList.contains(cls) : RegExp(`\\b${cls}\\b`).test(el.className);
			})
		},
		removeChildren: {
			get: () => (function () {
				nc.removeChildren(this);
			})
		},
		remove: {
			get: () => (function () {
		...
		includeHTML: {
			get: () => (function (ulr) {
				nc.includeHTML(this, ulr);
			})
		},
		getElementsByAttribute: {
			get: () => (function (attr, val) {
				return (this.length ? this[0] : this).querySelectorAll(val ? `*[${attr}~=${val}]` : `*[${attr}]`);
			})
		},
		getStyleProperty: {
			get: () => (function (prop) {
				return window.getComputedStyle((this.length ? this[0] : this)).getPropertyValue(prop);
			})
		}
	}
},
Object.defineProperties(HTMLElement.prototype, nc.funcs), Object.defineProperties(NodeList.prototype, nc.funcs),
Object.defineProperties(HTMLCollection.prototype, nc.funcs), Object.defineProperties(Array.prototype, nc.funcs),
...
require = (l => {
	...
	case "jquery":
		if (!window.jQuery)
			return require("https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js");
		break;

	case "bootstrap":
		return window.jQuery || require("Jquery"),
